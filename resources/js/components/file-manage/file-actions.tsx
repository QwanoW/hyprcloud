...
import { ChevronDown, FolderPlus, Plus, RotateCw, Share, Trash, Redo2} from 'lucide-react';
...
export function FileActions({
    ...
    return (
        <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
                <Button disabled={disableActions} onClick={() => onAction('remove-from-collection')} variant="destructive" className="gap-2">
                    <Redo2 className="h-4 w-4" />
                    {t('file_manage.actions_button_remove_from_collection')}
                </Button>
                <Button
                    disabled={disableActions || disableMultipleAction || selectedFileIds.length !== 1}
                    onClick={() => setShareDialogOpen(true)}
